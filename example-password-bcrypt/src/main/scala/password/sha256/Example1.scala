...
object Example1 extends App {
  ...
  def sequence = {
    ...
    (1 to 20).foreach { i => println("check" + i + "[" + createHash(password)) }
  ...
  def authenticate(password: String): Boolean = {
    ...
    val newPasswordHash = MessageDigest.getInstance("MD5").digest(password.getBytes).map("%02x".format(_)).mkString
